...
- P8X/OS v0.5 boots from CF: shell with `DIR`/`LOAD`/`RUN`/`SAVE`/`DEL`/`DUMP`/`DEP`/`PACK`/`HELP`; `DEP`+`SAVE`+`RUN` author and run programs on-target; `PACK` reclaims deleted space; host-side `p8xfs.py` builds + `fsck`s disk images (`make test-os`)
- BASIC builds three ways from one source: standalone, disk-bootable (`B`), and ROM-in-monitor (launched by `X`) (`make test-basic`)
- **Next:** P8XFS v2 hierarchy (`CD`/`MKDIR`/`TREE`, directories-as-files)
