...
- P8X/OS v0.8 reads flat **and hierarchical (P8XFS v2)** volumes: shell with `DIR [path]`/`CD`/`MKDIR`/`RMDIR`/`TREE`/`LOAD`/`RUN`/`SAVE`/`DEL`/`DUMP`/`DEP`/`PACK`; path resolution + CWD-path prompt; host-side `p8xfs.py` builds (`--v2`), navigates, and `fsck`s disk images (`make test-os`)
