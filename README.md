...
- P8X/OS v1.0 — full shell over flat **and hierarchical (P8XFS v2)** volumes. Built-in commands: `DIR [path]`/`CD`/`PWD`/`MKDIR`/`RMDIR`/`TREE`/`LOAD`/`RUN`/`SAVE`/`DEL`/`DUMP`/`DEP`/`PACK`/`FSCK`/`EXIT`. **Userland commands in `/BIN`** (written in C, run by bare name via implicit RUN + a `/BIN` search PATH, or explicit `RUN`): **`CAT`/`WC`/`GREP`/`CP`/`MV`/`HEAD`/`TAIL`/`MORE`/`SORT`/`UNIQ`/`SED`/`FIND`/`DIFF`** plus `DIR`/`PWD` (`-R`, etc.) — see [os/commands/](os/commands/README.md). Path resolution + CWD-path prompt; I/O redirection (`<`/`>`) and two-stage pipes (`a | b`); line editing (backspace/DEL, Ctrl-D EOF); **`PACK`** compacts the directory tree and **`FSCK`** checks integrity on-target; host-side `p8xfs.py` builds (`--v2`), navigates, and `fsck`s images (`make test-os`)
