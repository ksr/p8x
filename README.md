...
- P8X/OS v1.0 — full shell over flat **and hierarchical (P8XFS v2)** volumes. Built-in commands: `CD`/`MKDIR`/`RMDIR`/`LOAD`/`RUN`/`SAVE`/`DEL`/`DUMP`/`DEP`/`PATH`/`PACK`/`IMPORT`/`FSCK`/`FORMAT`/`HELP`/`EXIT` (the minimal-kernel split moved the pure-viewer commands to `/BIN`; `DUMP` stays native — as a `/BIN` program it would load into the `$7A00` TPA and overwrite the very memory it dumps). **Dual CompactFlash** — two full P8XFS volumes, drive `0:` (boot/default) and `1:`, each with its own CWD; a bare `1:`/`0:` switches the current drive, a `0:`/`1:` path prefix targets a drive per-command on the built-ins, and `IMPORT N:/dir` bulk-copies another drive's directory into the CWD (card provisioning). **Userland commands in `/BIN`** (written in C, run by bare name via implicit RUN + a `/BIN` search PATH, or explicit `RUN`): **`DIR`/`PWD`/`TREE`/`CAT`/`WC`/`GREP`/`CP`/`MV`/`HEAD`/`TAIL`/`MORE`/`SORT`/`UNIQ`/`SED`/`FIND`/`DIFF`** (`DIR -R`, etc.) — see [os/commands/](os/commands/README.md). Path resolution + CWD-path prompt; I/O redirection (`<`/`>`) and two-stage pipes (`a | b`); line editing (backspace/DEL, Ctrl-D EOF); **`PACK`** compacts the directory tree and **`FSCK`** checks integrity on-target; host-side `p8xfs.py` builds (`--v2`), navigates, and `fsck`s images (`make test-os`)
