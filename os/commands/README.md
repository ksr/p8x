...
| [`desk.c`](desk.c) | `desk` | **Overlapping windows** — rung 1 of the window system: two draggable windows (mouse title-bar drag with a complement outline, TAB focus), painter's-algorithm repaint, and per-window **hardware clipping** via a WINDOW/VWPORT pair per window (local coords, the card clips). One window is a small **terminal** — card-TEXT lines, typed input when focused. Shares [`lib_ptr.c`](lib_ptr.c) (console keyboard + xterm-mouse events) with `paint`. `man desk`. C-only. |
