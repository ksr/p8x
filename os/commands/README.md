...
| [`cube.c`](cube.c) | `cube [frames]` | Spinning perspective **wireframe cube** — the stage-7 3D demo and first client of `lib_g3d` (`//#use gfx` + `//#use g3d`). 64 frames (one full turn) by default, ~72 ms a frame (~14 fps); rebuilt from constants each frame so it never accumulates rounding error. Needs the display (`?No display` without). C-only, and (like `disasm`) outside the `p8cc.c` self-host subset: its sine/edge tables are brace-initialized arrays. |
...
| [`lib_g3d.c`](lib_g3d.c) | **Wireframe 3D** (STAGE7-DESIGN.md): retained edge pool (`g3line`, 512 edges), `g3window`/`g3view` (window-space clipping makes viewports true clip rectangles), `g3persp`, `g3render` (near clip → project → Cohen-Sutherland → viewport map → hardware LINE). Foundation: `muldiv(a,b,c)` — signed (a\*b)/c through a 32-bit intermediate, with a native-`*`/`/` fast path when the product fits 16 bits (~10× cheaper, and nearly every real call). Needs `//#use gfx` above it. | `cube` |
