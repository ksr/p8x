...
> the `cpp | lex | cc1` passes below — the earlier path-A front end that ran only
> the front half on-target — are **DEPRECATED and no longer built/shipped**;
> `cc` compiles the whole thing on the machine.
