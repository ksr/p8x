...
| [`finder.c`](finder.c) | `finder [dir]` | **Two-mode Finder desktop** (P4): a full-screen file browser with a menu bar and a scrolling file list (dirs cyan, selection a yellow bar). Arrows/ENTER navigate a dir, launch a `.BIN` full-screen (`SYS_EXEC`), or open a `.P8I` in image's VIEW mode; **`a`** drops the Apps menu (paint/term/write/…); Backspace goes up, `q` quits to the shell. Claims the screen (`GTSUSP`) and sets its own GL ground state (`WINDOW`/`VWPORT`/`PROJCT 0`); decodes arrow ESC-sequences itself. Launches auto-return via a `SYS_RUNSH` script chain (no run-and-return syscall). `man finder`. C-only. |
...
| [`screen.c`](screen.c) | `screen on\|off` | **Glass-TTY switch** (P2): turn the on-screen text console on or off — `on` mirrors `CONOUT` onto the GL display (clears + homes first), `off` goes back to serial only, no arg reports the state. Opt-in and off by default (the graphics ecosystem shares the one screen). `man screen`. C-only (asm twin backlogged). |
