...
ensure_src() {
    ...
    _ccmds="cube tri rotate page camera gl md house clsave paint desk wdesk finder"
...
if [ ! -f "$disk" ]; then
    ...
    for ex in dir pwd cat wc grep cp mv del help head tail more sort uniq sed find diff tree vi touch man dep dump examine disasm awk cmp cube tri rotate page camera gl md house clsave paint desk wdesk finder image; do
    ...
    for ex in cube tri rotate page camera gl md house clsave paint desk wdesk finder; do
        python3 "$root/tools/p8xfs.py" put "$disk" "$build/$ex.bin" \
