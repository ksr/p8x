...
if [ ! -f "$disk" ]; then
    ...
    printf '        .org $6300\n        LDA #%cH%c\n        JSR $0103\n        LDA #%cI%c\n        JSR $0103\n        LDA #$0D\n        JSR $0103\n        LDA #$0A\n        JSR $0103\n        RTS\n' "'" "'" "'" "'" > "$build/hi.asm"
    python3 "$root/assembler/p8xasm.py" "$build/hi.asm" -o "$build/hi.bin" --base 0x6300 >/dev/null
    ...
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/basicrun.bin" \
        --name /bin/basic.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/assembler/p8xasm.py" "$build/basicc.asm" -o "$build/basicc.bin" --base 0x6300 >/dev/null
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/basicc.bin" \
        --name /binc/basic.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/assembler/p8xasm.py" "$root/apps/p8xedit.asm" -o "$build/edit.bin" \
        --base 0x6300 >/dev/null
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/edit.bin" \
        --name /bin/edit.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/asm.bin" \
        --name /bin/asm.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/cc.bin" \
        --name /bin/cc.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/assembler/p8xasm.py" "$build/asmc.asm" -o "$build/asmc.bin" --base 0x6300 >/dev/null
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/asmc.bin" \
        --name /binc/asm.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    python3 "$root/assembler/p8xasm.py" "$build/ccc.asm" -o "$build/ccc.bin" --base 0x6300 >/dev/null
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/ccc.bin" \
        --name /binc/cc.bin --load 0x6300 --exec 0x6300 >/dev/null
    ...
    for ex in dir pwd cat wc grep cp mv del help head tail more sort uniq sed find diff tree vi touch man dep dump examine disasm awk cmp cube tri rotate page camera gl md house clsave paint desk wdesk finder term write kermit screen image; do
        ...
        python3 "$root/assembler/p8xasm.py" "$build/$ex.asm" -o "$build/$ex.bin" --base 0x6300 >/dev/null
        python3 "$root/tools/p8xfs.py" put "$disk" "$build/$ex.bin" \
            --name "/binc/$ex.bin" --load 0x6300 --exec 0x6300 >/dev/null
    done
    ...
    for ex in cube tri rotate page camera gl md house clsave paint desk wdesk finder term write kermit screen; do
        python3 "$root/tools/p8xfs.py" put "$disk" "$build/$ex.bin" \
            --name "/bin/$ex.bin" --load 0x6300 --exec 0x6300 >/dev/null
    done
    ...
    for ex in dir pwd cat wc grep cp mv head tail more sort uniq sed find diff tree vi touch man dep dump examine disasm awk cmp image del help; do
        ...
        python3 "$root/assembler/p8xasm.py" "$build/$ex.a.asm" -o "$build/$ex.a.bin" --base 0x6300 >/dev/null
    ...
    _mddoc docs/p8x-two-mode-design.md   TWOMODE.MD
    ...
    # /examples: the demo assets, out of the root's way.
    python3 "$root/tools/p8xfs.py" mkdir "$disk" /examples >/dev/null
    # The demo photo for BASIC's IMAGE statement (P8I, made with tools/p8img.py):
    # IMAGE 112,8,"/examples/MANDRILL.P8I". Tracked so a fresh disk has it.
    python3 "$root/tools/p8xfs.py" put "$disk" "$root/os/mandrill.p8i" \
        --name /examples/MANDRILL.P8I >/dev/null
    # The PG-640A manual's house as an ASCII scene file (ch.3; the `house`
    # command embeds the same bytes): `gl /examples/HOUSE.GL` records it
    # into command list 2, then VWP/DS/CLRUN 2 draws it (man gl). Tracked
    # here for the same reason as the mandrill.
    python3 "$root/tools/p8xfs.py" put "$disk" "$root/os/house.gl" \
        --name /examples/HOUSE.GL >/dev/null
    # The stage-10h stroke font (generated by generators/gen_font.py):
    # `gl /FONT.GL` loads it into the card's glyph bank -- SDRAM is
    # volatile, so a session that wants TEXT streams this first.
    python3 "$root/tools/p8xfs.py" put "$disk" "$root/os/font.gl" \
        --name /FONT.GL >/dev/null
    # A sample assembly source so the EDIT -> ASM -> RUN loop is demoable out
    # of the box: cd /examples, then edit hello.asm (look/edit), then
...
ASMEOF
    python3 "$root/tools/p8xfs.py" put "$disk" "$build/hello.asm" --name /examples/hello.asm >/dev/null
