...
#
# Both are assembled with the SAME assembler (assembler/p8xasm.py, --base 0x5900)
...
for cmd in $ALL; do
    ...
    if [ -f "$CDIR/$cmd.c" ]; then
        ...
        $ASM "$WORK/${cmd}_c.asm" -o "$WORK/${cmd}_c.bin" --base 0x5900 >/dev/null 2>&1
        csz=$(wc -c < "$WORK/${cmd}_c.bin" | tr -d ' ')
    fi
    if [ -f "$ADIR/$cmd.asm" ]; then
        sh "$ADIR/mkasm.sh" "$cmd" > "$WORK/${cmd}_full.asm"
        $ASM "$WORK/${cmd}_full.asm" -o "$WORK/${cmd}_a.bin" --base 0x5900 >/dev/null 2>&1
