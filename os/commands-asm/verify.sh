...
cmd_script() {
    case "$1" in
        ...
        # same bytes — the program itself loads at $5900 and would differ.
        ...
        # dump a zeroed RAM page clear of the program at $5900 (identical in
...
build_c() { # $1=name -> $W/<name>_h.bin
    ...
    $ASM "$W/$1.hlp.asm" -o "$W/$1_h.bin" --base 0x5900 >/dev/null 2>&1
}
build_c cat; build_c find
install_helpers() { # $1=img  $2=name-under-test (lowercase, skip that one)
    for h in cat find; do
        [ "$h" = "$2" ] && continue
        $FS put "$1" "$W/${h}_h.bin" --name "/bin/$h.bin" --load 0x5900 --exec 0x5900 >/dev/null
    done
}
...
for cmd in $ALL; do
    ...
    $ASM "$W/${cmd}_c.asm" -o "$W/${cmd}_c.bin" --base 0x5900 >/dev/null 2>&1
    sh "$ADIR/mkasm.sh" "$cmd" > "$W/${cmd}_full.asm"
    $ASM "$W/${cmd}_full.asm" -o "$W/${cmd}_a.bin" --base 0x5900 >/dev/null 2>&1
    scr=$(cmd_script "$up")
    fixtures "$W/dc.img"; install_helpers "$W/dc.img" "$cmd"; $FS put "$W/dc.img" "$W/${cmd}_c.bin" --name "/bin/$cmd.bin" --load 0x5900 --exec 0x5900 >/dev/null
    fixtures "$W/da.img"; install_helpers "$W/da.img" "$cmd"; $FS put "$W/da.img" "$W/${cmd}_a.bin" --name "/bin/$cmd.bin" --load 0x5900 --exec 0x5900 >/dev/null
