...
(`$6100`), same argument ABI (`P2` = arg-tail pointer), same OS/BIOS calls, so it
...
| call | addr | in | out |
|------|------|----|-----|
| entry | `$6100` | `P2` = ptr to NUL-terminated arg tail | `RTS` to OS |
...
by hand — both through the same `p8xasm.py --base 0x6100` — and prints a
