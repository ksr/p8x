...
wk_sink: LDB  #255
        ...
        STA  sinklst
        LDW kb,#29                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub                     ; kw = h - 29
        MOVW sinky,kw                ; <- tierA: word move (next: LDA)
...
sink_home:
        LDA  #18                        ; MOVE3
        JSR  kput
        LDW kw,#4                ; <- tierA: word constant (next: JSR ksw)
        JSR  ksw
        MOVW kw,sinky                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        LDW kw,#0                ; <- tierA: zero word (next: JSR ksw)
        JSR  ksw
        RTS
...
OUTWIN: LDA  RCH
        ...
        JSR  kput
ow_ret: RTS
ow_lf:  MOVW ka,sinky                ; <- tierA: word move (next: LDA)
        LDW kb,#13                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        MOVW sinky,kw                ; <- tierA: word move (next: JSR sink_home)
        JSR  sink_home
        RTS
ow_cr:  JSR  sink_home
        RTS
...
wru_esc:JSR  $0100                      ; expect '['
        ...
        JZ   wru_lf
        JMP  wev_h1
wru_up: LDW kdxy,#8                ; <- tierA: word constant (next: LDA)
        LDA  #2                         ; field = y
        JMP  wru_mv
wru_dn: LDW kdxy,#65528                ; <- tierA: word constant (next: LDA)
        LDA  #2
        JMP  wru_mv
wru_rt: LDW kdxy,#8                ; <- tierA: word constant (next: LDA)
        LDA  #0                         ; field = x
        JMP  wru_mv
wru_lf: LDW kdxy,#65528                ; <- tierA: word constant (next: LDA)
        LDA  #0
...
wru_mouse:
        ...
        LDW kb,#6                ; <- tierA: word constant (next: JSR k_mul)
        JSR  k_mul                      ; kw = (x-1)*6
        MOVW kmx,kw                ; <- tierA: word move (next: JSR k_rdnum)
        JSR  k_rdnum                    ; y -> knum ; term M/m
        LDA  knum                       ; save the y CELL (menu-bar test)
        STA  kcelly
        JSR  k_dec1n                    ; ka = knum - 1
        LDW kb,#11                ; <- tierA: word constant (next: JSR k_mul)
        JSR  k_mul                      ; kw = (y-1)*11
        LDW ka,#271                ; <- tierA: word constant (next: LDA)
        MOVW kb,kw                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        MOVW kmy,kw                ; <- tierA: word move (next: LDA)
...
wmp_win:JSR  k_hit                      ; which window is under the cursor?
        ...
        MOVW ka,ky                ; <- tierA: word move (next: LDA)
        MOVW kb,kch                ; <- tierA: word move (next: JSR k16add)
        JSR  k16add
        MOVW ka,kw                ; <- tierA: word move (next: LDA)
        LDW kb,#14                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        MOVW ka,kmy                ; <- tierA: word move (next: LDA)
        MOVW kb,kw                ; <- tierA: word move (next: JSR k_ge)
        JSR  k_ge
        JZ   wru_grab                   ; a body press: just grab
        MOVW ka,kx                ; <- tierA: word move (next: LDA)
        LDA  #3
        JSR  k_off16
        MOVW kmx2,kw                ; <- tierA: word move (next: LDA)
        LDA  #11
        JSR  k_off16                    ; ka is still kx
        MOVW kmy2,kw                ; <- tierA: word move (next: LDA)
        MOVW ka,kmx                ; <- tierA: word move (next: LDA)
        MOVW kb,kmx2                ; <- tierA: word move (next: JSR k_ge)
        JSR  k_ge
        JZ   wru_grab                   ; left of the box
        MOVW ka,kmy2                ; <- tierA: word move (next: LDA)
        MOVW kb,kmx                ; <- tierA: word move (next: JSR k_ge)
        JSR  k_ge
...
wru_grab:
        ...
        STA  kdragw
        MOVW ka,kmx                ; <- tierA: word move (next: LDA)
        MOVW kb,kx                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        MOVW kgx,kw                ; <- tierA: word move (next: LDA)
        MOVW ka,kmy                ; <- tierA: word move (next: LDA)
        MOVW kb,ky                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        LDA  kw
        STA  kgy
        LDA  kw+1
...
k_inwin:JSR  koff
        ...
        ; kmx >= kx ?
        MOVW ka,kmx                ; <- tierA: word move (next: LDA)
        MOVW kb,kx                ; <- tierA: word move (next: JSR k_ge)
        JSR  k_ge
        JZ   kit_no
        ; kmx < kx+kcw ?  (reject if kmx >= kx+kcw)
        MOVW ka,kx                ; <- tierA: word move (next: LDA)
        MOVW kb,kcw                ; <- tierA: word move (next: JSR k16add)
        ...
        ; kmy >= ky ?
        MOVW ka,kmy                ; <- tierA: word move (next: LDA)
        MOVW kb,ky                ; <- tierA: word move (next: JSR k_ge)
        JSR  k_ge
        JZ   kit_no
        ; kmy < ky+kch ?
        MOVW ka,ky                ; <- tierA: word move (next: LDA)
...
k_dragmove:
        MOVW ka,kmx                ; <- tierA: word move (next: LDA)
        MOVW kb,kgx                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        MOVW kmx2,kw                ; <- tierA: word move (next: LDA)
        MOVW ka,kmy                ; <- tierA: word move (next: LDA)
        MOVW kb,kgy                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        MOVW kmy2,kw                ; <- tierA: word move (next: LDA)
...
krn_lp: JSR  $0100
        ...
        SUB
        STA  kt                         ; kt = 0..9
        MOVW ka,knum                ; <- tierA: word move (next: LDA)
        LDW kb,#10                ; <- tierA: word constant (next: JSR k_mul)
...
k_movetop:
        ...
        LDA  (P1)
        STA  ka+1
        MOVW kb,kdxy                ; <- tierA: word move (next: JSR k16add)
...
wk_draw:JSR  koff                       ; A = 24*ki
        ...
        JSR  k16add                     ; kw = kx+kcw
        JSR  kw_dec1                    ; kw -= 1
        MOVW kx1,kw                ; <- tierA: word move (next: LDA)
        MOVW ka,ky                ; <- tierA: word move (next: LDA)
        MOVW kb,kch                ; <- tierA: word move (next: JSR k16add)
        JSR  k16add
        JSR  kw_dec1
        MOVW ky1,kw                ; <- tierA: word move (next: LDA)
        ...
        LDW kb,#14                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        MOVW kty,kw                ; <- tierA: word move (next: LDA)
...
wkd_bar:LDA  #$10                       ; MOVE(kx, kty)
        JSR  kput
        MOVW kw,kx                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        MOVW kw,kty                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        ...
        JSR  k_off16
        JSR  ksw
        MOVW ka,kty                ; <- tierA: word move (next: LDA)
        LDA  #2
        JSR  k_off16
        JSR  ksw
        ...
        ; anchor x = kx + 16
        MOVW ka,kx                ; <- tierA: word move (next: LDA)
        LDW kb,#16                ; <- tierA: word constant (next: JSR k16add)
        JSR  k16add
        MOVW ktx,kw                ; <- tierA: word move (next: LDA)
        ; anchor y = ky + kch - 11
        MOVW ka,ky                ; <- tierA: word move (next: LDA)
        MOVW kb,kch                ; <- tierA: word move (next: JSR k16add)
        JSR  k16add                     ; ky+kch
        MOVW ka,kw                ; <- tierA: word move (next: LDA)
        LDW kb,#11                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub                     ; ky+kch-11
        MOVW kty,kw                ; <- tierA: word move (next: LDA)
        ; MOVE3 ktx kty 0
        LDA  #$12
        JSR  kput
        MOVW kw,ktx                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        MOVW kw,kty                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        LDW kw,#0                ; <- tierA: zero word (next: JSR ksw)
...
k_content:
        ...
        LDW kw,#0                ; <- tierA: zero word (next: JSR ksw)
        JSR  ksw
        MOVW ka,kcw                ; <- tierA: word move (next: LDA)
        LDW kb,#3                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        JSR  ksw
        LDW kw,#0                ; <- tierA: zero word (next: JSR ksw)
        JSR  ksw
        MOVW ka,kch                ; <- tierA: word move (next: LDA)
        LDW kb,#16                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        JSR  ksw
        LDA  #$B2                       ; VWPORT vx1 vx2 vy1 vy2
        JSR  kput
        MOVW ka,kx                ; <- tierA: word move (next: LDA)
        LDW kb,#1                ; <- tierA: word constant (next: JSR k16add)
        JSR  k16add
        JSR  ksw
        MOVW ka,kx                ; <- tierA: word move (next: LDA)
        MOVW kb,kcw                ; <- tierA: word move (next: JSR k16add)
        JSR  k16add
        MOVW ka,kw                ; <- tierA: word move (next: LDA)
        LDW kb,#2                ; <- tierA: word constant (next: JSR k16sub)
        JSR  k16sub
        JSR  ksw
        LDW ka,#286                ; <- tierA: word constant (next: LDA)
        MOVW kb,ky                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub                     ; 286 - ky
        MOVW ka,kw                ; <- tierA: word move (next: LDA)
        MOVW kb,kch                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub                     ; - kch
        JSR  ksw
        LDW ka,#270                ; <- tierA: word constant (next: LDA)
        MOVW kb,ky                ; <- tierA: word move (next: JSR k16sub)
        JSR  k16sub
        JSR  ksw
        RTS
...
kmove_xy:
        ...
        MOVW kw,kx                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        MOVW kw,ky                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        RTS
; RECT(kx1,ky1)
krect_11:
        LDA  #$34
        JSR  kput
        MOVW kw,kx1                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        MOVW kw,ky1                ; <- tierA: word move (next: JSR ksw)
        JSR  ksw
        RTS
...
k_ident:LDP1 #kid                ; <- tierA: pointer constant (next: LDA)
        LDA  #18
        STA  kt
kid_lp: LDA  (P1)+
        JSR  kput
