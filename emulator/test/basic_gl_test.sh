...
printf 'B\rbasic\r10 GL "RESETF CLEARS 0 0 0"\r20 GL "WINDOW -120 120 -120 120"\r30 GL "VWPORT 104 375 0 271"\r40 GL "COLOR 0 63 0 PRMFIL 1"\r50 A=35\r60 GL "MDY "+STR$(A)\r70 GL "POLY3 3 -8 -8 30 8 -8 30 0 4 42"\r80 PRINT PIXELR(364,204)\r90 END\rRUN\rNEW\r10 RESETF\r20 CLEARS 0,0,0\r30 WINDOW -120,120,-120,120\r40 VWPORT 104,375,0,271\r50 COLOR RGB(31,0,0)\r60 PRMFIL 1\r70 CLBEG 1\r80 MDROTY 5\r90 CLEND\r100 CLOOP 1,3+4\r110 POLY3 3,-80,-80,300,80,-80,300,0,40,420\r120 PRINT PIXELR(364,204)\r130 END\rRUN\rNEW\r10 RESETF\r20 CLEARS 0,0,0\r30 WINDOW 0,479,0,271\r40 VWPORT 0,479,0,271\r50 COLOR RGB(31,63,0)\r60 MOVE 100,100\r70 RECT 200,150\r80 MOVE 150,125\r90 AREA\r100 COLOR RGB(0,0,31)\r110 POLY 4,300,200,350,150,400,200,350,250\r120 COLOR RGB(0,63,31)\r130 MOVE 350,200\r140 AREABC 0,0,31\r150 PRINT PIXELR(150,146)\r160 PRINT PIXELR(320,71)\r170 END\rRUN\rNEW\r10 RESETF : CLEARS 0,0,0 : PROJCT 0\r20 WINDOW 0,479,0,271\r30 VWPORT 0,479,0,271\r40 TDEFIN 65\r50 MOVER3 0,0,0 : DRAWR3 0,6,0\r60 MOVER3 6,-6,0\r70 CLEND\r80 COLOR RGB(31,63,0)\r90 TSIZE 512 : TANGLE 0\r100 MOVE3 100,100,0\r110 TEXT "A"\r120 PRINT PIXELR(200,71)\r130 PRINT PEEK(65363)\r140 END\rRUN\rNEW\r10 RESETF : CLEARS 0,0,0 : PROJCT 0\r20 WINDOW 0,479,0,271\r30 VWPORT 0,479,0,271\r40 COLOR RGB(31,0,0)\r50 MOVE 100,136 : ELIPSE 60,30\r60 LINPAT -4096\r70 MOVE 100,60 : DRAW 300,60\r80 LINPAT -1\r90 COLOR RGB(0,63,0) : PRMFIL 1\r100 MOVE 350,136 : SECTOR 50,0,180\r110 PRMFIL 0\r120 TJUST 2,2 : TJUST 1,1\r130 PRINT PEEK(65363)\r140 END\rRUN\rBYE\r' > bgl.in
...
tr -d '\0\r' < bgl.out | grep -q "^2016$" || fail "PIXELR(364,204) != 2016 (string-form tri missing)"
...
tr -d '\0\r' < bgl.out | grep -q -- "^-2048$" || fail "PIXELR(364,204) != -2048 (native verbs missing)"
...
tr -d '\0\r' < bgl.out | grep -q -- "^-32$" || fail "PIXELR(150,146) != -32 (AREA fill missing)"
tr -d '\0\r' < bgl.out | grep -q "^2047$" || fail "PIXELR(320,71) != 2047 (AREABC fill missing)"
...
# program 5 owns the final frame: ELIPSE, a dashed LINPAT line, a filled
# SECTOR, and TJUST round-tripping without error -- all native statements
python3 - <<'EOF' || exit 1
...
assert p(350,156)==GREEN, "SECTOR interior: %r"%(p(350,156),)
assert p(350,120)==BLACK, "SECTOR filled below its sweep"
EOF
echo "BASIC-GL TEST: PASS (GL statement + native verbs + recorded-list rotation + COLOR bridge + PIXELR + AREA/AREABC + TDEFIN/TSIZE/TEXT + ELIPSE/SECTOR/LINPAT/TJUST)"
