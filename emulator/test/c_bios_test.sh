...
run() {   # $1 = asm file -> emulator output (letters only), feeding 'Q' to getchar
    python3 $ROOT/assembler/p8xasm.py "$1" -o cbios.bin --base 0x6100 >/dev/null
    rm -f cbios.img
    python3 $ROOT/tools/p8xfs.py create cbios.img >/dev/null
    python3 $ROOT/tools/p8xfs.py boot   cbios.img osc.bin >/dev/null
    python3 $ROOT/tools/p8xfs.py put    cbios.img cbios.bin --name CB.bin --load 0x6100 --exec 0x6100 >/dev/null
