...
python3 $ROOT/assembler/p8xasm.py $ROOT/apps/p8xedit.asm -o edit.bin --base 0x6300 >/dev/null

rm -f ed.img
python3 $ROOT/tools/p8xfs.py create ed.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   ed.img ose.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    ed.img edit.bin --name edit.bin --load 0x6300 --exec 0x6300 >/dev/null
