...
python3 $ROOT/assembler/p8xasm.py $ROOT/os/p8xos.asm -o bpos.bin --base 0x2000 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py bpdir.asm -o bpdir.bin --base 0x6300 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    bp.img bpdir.bin --name /bin/dir.bin --load 0x6300 --exec 0x6300 >/dev/null
# dep/save seed F.bin's content below; dep is now a /bin program, so install it.
python3 $ROOT/tools/clib.py $ROOT/os/commands/dep.c -o bpdep.pp.c >/dev/null
python3 $ROOT/compiler/p8cc.py bpdep.pp.c -o bpdep.asm >/dev/null
python3 $ROOT/assembler/p8xasm.py bpdep.asm -o bpdep.bin --base 0x6300 >/dev/null
python3 $ROOT/tools/p8xfs.py put    bp.img bpdep.bin --name /bin/dep.bin --load 0x6300 --exec 0x6300 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py bpdel.asm -o bpdel.bin --base 0x6300 >/dev/null
python3 $ROOT/tools/p8xfs.py put    bp.img bpdel.bin --name /bin/del.bin --load 0x6300 --exec 0x6300 >/dev/null
