...
check() {   # $1 = label, $2 = asm file
    python3 $ROOT/assembler/p8xasm.py "$2" -o r.bin --base 0x6300 >/dev/null
    rm -f r.img
    python3 $ROOT/tools/p8xfs.py create r.img >/dev/null
    python3 $ROOT/tools/p8xfs.py boot   r.img osc.bin >/dev/null
    python3 $ROOT/tools/p8xfs.py put    r.img r.bin --name R.bin --load 0x6300 --exec 0x6300 >/dev/null
