...
#   - the native toolchain: `sh /src/mk/pwd.sh` rebuilds pwd (C + asm) on-target,
...
$ROOT/tools/p8xfs.py ls "$D0" /src/mk 2>/dev/null | grep -qE '^pwd\.sh ' || fail "/src/mk/pwd.sh (build script) missing"
...
printf 'B\rdir /bin\rcat README.TXT | grep hello\rsh /src/mk/pwd.sh\rrun /src/commands/c/bin/pwd.bin\rcat /d1/DATA/NOTES.TXT\r' \
...
$ROOT/tools/p8xfs.py ls "$D0" /src/commands/c/bin   2>/dev/null | grep -qE '^pwd\.bin ' || fail "sh mk/pwd.sh: no C binary"
$ROOT/tools/p8xfs.py ls "$D0" /src/commands/asm/bin 2>/dev/null | grep -qE '^pwd\.bin ' || fail "sh mk/pwd.sh: no asm binary"
