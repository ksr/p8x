...
build_cmd() {   # $1 = command name -> $1.bin in /bin of mn0
    ...
    python3 $ROOT/assembler/p8xasm.py mn_$1.asm -o mn_$1.bin --base 0x6300 >/dev/null
}
...
for c in cat dir cp; do
    build_cmd $c
    U=$(echo $c | tr a-z A-Z)
    python3 $ROOT/tools/p8xfs.py put mn0.img mn_$c.bin --name /bin/$c.bin --load 0x6300 --exec 0x6300 >/dev/null
done
