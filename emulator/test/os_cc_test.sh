...
python3 $ROOT/assembler/p8xasm.py $ROOT/os/p8xos.asm -o oscc.bin --base 0x2000 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    cc.img cc.bin  --name /bin/cc.bin  --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put    cc.img asm.bin --name /bin/asm.bin --load 0x6100 --exec 0x6100 >/dev/null
# exercises MUTUAL RECURSION via a forward prototype (even/odd) plus a string
# printed with the puts() builtin. puts("P8") -> "P8\n"; even(10) is true -> 'A'.
# Expected: a line "P8" then a line "A".
...
{ grep -q '^P8$' cc_out.txt && grep -q '^A$' cc_out.txt; } || { echo "--- transcript ---"; sed -n '/PROG/,$p' cc_out.txt | head; fail "compiled program did not print P8 then A"; }
