...
python3 $ROOT/assembler/p8xasm.py cs.asm -o cs.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/clib.py $ROOT/os/commands/gl.c -o csgl.c
python3 $ROOT/compiler/p8cc.py csgl.c -o csgl.asm >/dev/null
python3 $ROOT/assembler/p8xasm.py csgl.asm -o csgl.bin --base 0x5900 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    cs.img cs.bin  --name /bin/clsave.bin --load 0x5900 --exec 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put    cs.img csgl.bin --name /bin/gl.bin --load 0x5900 --exec 0x5900 >/dev/null
