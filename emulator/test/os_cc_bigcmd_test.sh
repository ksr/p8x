...
python3 $ROOT/assembler/p8xasm.py $ROOT/apps/p8xcc.asm -o ccbc.bin --base 0x6300 >/dev/null
python3 $ROOT/generators/gen_p8xopc.py > opctab.asm
cat $ROOT/apps/p8xasm.asm opctab.asm > asmfull.asm
python3 $ROOT/assembler/p8xasm.py asmfull.asm -o asmbc.bin --base 0x6300 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    bc.img ccbc.bin  --name /bin/cc.bin  --load 0x6300 --exec 0x6300 >/dev/null
python3 $ROOT/tools/p8xfs.py put    bc.img asmbc.bin --name /bin/asm.bin --load 0x6300 --exec 0x6300 >/dev/null
