...
python3 $ROOT/assembler/p8xasm.py asmc.asm -o asmc.bin --base 0x6300 >/dev/null
[ "$(wc -c < asmc.bin)" -le 17664 ] || fail "asmc.bin is $(wc -c < asmc.bin) bytes, overlaps the symbol table at \$A800"
...
python3 $ROOT/assembler/p8xasm.py acself.asm -o acselfgold.bin --base 0x6300 >/dev/null
sh $ROOT/os/commands-asm/mkasm.sh cat > accat.asm
python3 $ROOT/assembler/p8xasm.py accat.asm -o accatgold.bin --base 0x6300 >/dev/null
rm -f ac.img
...
python3 $ROOT/tools/p8xfs.py put ac.img asmc.bin --name /bin/asmc.bin --load 0x6300 --exec 0x6300 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py act.asm -o actgold.bin --base 0x6300 -D FOO=0x41 >/dev/null 2>&1 || \
    { printf 'FOO = $41\n' | cat - act.asm | grep -v '.include' > actflat.asm; python3 $ROOT/assembler/p8xasm.py actflat.asm -o actgold.bin --base 0x6300 >/dev/null; }
