...
build_disk() {   # $1 dir.bin  $2 pwd.bin -> dir.img with /SUB/X.DAT + both programs
    ...
    python3 $ROOT/tools/p8xfs.py put    dir.img "$1" --name DIR.bin --load 0x6100 --exec 0x6100 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    dir.img "$2" --name PWD.bin --load 0x6100 --exec 0x6100 >/dev/null
}
...
compile_one() {   # $1 = compiler tag: build both programs with it
    ...
    python3 $ROOT/tools/clib.py $ROOT/os/commands/pwd.c -o p.pp.c   # splice //#use fsread
    if [ "$1" = "host" ]; then
        ./p8cc_host < d.pp.c > d.asm
        ./p8cc_host < p.pp.c > p.asm
    else
        python3 $ROOT/compiler/p8cc.py d.pp.c -o d.asm >/dev/null
        python3 $ROOT/compiler/p8cc.py p.pp.c -o p.asm >/dev/null
    fi
    python3 $ROOT/assembler/p8xasm.py d.asm -o d.bin --base 0x6100 >/dev/null
