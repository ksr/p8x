...
build_disk() {   # $1 dir.bin  $2 pwd.bin -> dir.img with /SUB/X.DAT + both programs
    ...
    python3 $ROOT/tools/p8xfs.py put    dir.img "$1" --name DIR.bin --load 0x6300 --exec 0x6300 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    dir.img "$2" --name PWD.bin --load 0x6300 --exec 0x6300 >/dev/null
}
...
compile_one() {   # $1 = compiler tag: build both programs with it
    ...
    python3 $ROOT/assembler/p8xasm.py d.asm -o d.bin --base 0x6300 >/dev/null
