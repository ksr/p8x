...
cat > wk_stub.c <<EOF
char param[22];
int setw(int x, int y, int w, int h, char *t) {
    int i;
...
int main() {
    ...
    setw(40, 40, 210, 150, "SHAPES");
    setw(190, 90, 240, 140, "TERM");
...
for name in ("wk1.ppm", "wk2.ppm"):
    ...
    assert p(a,120,110)==(0,0,0),       "%s: SHAPES body not black" % name
...
print("frame 1 (stub) and frame 2 (resident redraw, stub GONE): two windows, desktop, titles")
EOF
