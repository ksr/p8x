...
python3 $ROOT/assembler/p8xasm.py $ROOT/os/p8xos.asm -o apos.bin --base 0x2000 >/dev/null
python3 $ROOT/tools/clib.py $ROOT/os/commands/cat.c -o apcat.c
python3 $ROOT/compiler/p8cc.py apcat.c -o apcat.asm >/dev/null
python3 $ROOT/assembler/p8xasm.py apcat.asm -o apcat.bin --base 0x5900 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    ap.img apcat.bin --name /bin/cat.bin --load 0x5900 --exec 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py aphelp.asm -o aphelp.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put    ap.img aphelp.bin --name /bin/help.bin --load 0x5900 --exec 0x5900 >/dev/null
printf 'oneline\n' > apin.dat
