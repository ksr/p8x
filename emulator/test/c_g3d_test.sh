...
#   2. cube frame 0 spot pixels (software path) -- a host replica of the whole pipeline
...
python3 $ROOT/assembler/p8xasm.py g3_cube.asm -o g3_cube.bin --base 0x6300 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py g3_md.asm -o g3_md.bin --base 0x6300 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    g3d.img g3_cube.bin --name /bin/cube.bin --load 0x6300 --exec 0x6300 >/dev/null
python3 $ROOT/tools/p8xfs.py put    g3d.img g3_md.bin   --name /bin/g3md.bin --load 0x6300 --exec 0x6300 >/dev/null
...
# (parts 4-6, the record-engine identity/cube/flip tests, retired with
#  the $FF40 interface at stage 10b -- the GL suites carry those checks:
#  c_gl_test flip semantics, tb_gl span exactness, tb_gl_pix/mpx pixels.)
...
echo "C-G3D TEST: PASS (muldiv vectors, software cube frame-0 spot pixels, native parity)"
