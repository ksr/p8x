...
asm() { python3 $ROOT/assembler/p8xasm.py "$1" -o "$2" --base 0x6300 >/dev/null; }
...
build_disk() {   # compile wc/grep/cat with $1 (py|host), build a disk
    ...
    python3 $ROOT/tools/p8xfs.py put    flt.img wc.bin   --name /bin/wc.bin   --load 0x6300 --exec 0x6300 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    flt.img grep.bin --name /bin/grep.bin --load 0x6300 --exec 0x6300 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    flt.img cat.bin  --name /bin/cat.bin  --load 0x6300 --exec 0x6300 >/dev/null
    printf 'alpha\r\nbeta\r\ngamma alpha\r\n' > tf.dat
