...
for c in dir cat; do
    ...
    python3 $ROOT/assembler/p8xasm.py mc_$c.asm -o mc_$c.bin --base 0x5900 >/dev/null
done
...
mk0() {   # fresh boot disk with /bin/{DIR,CAT} + /d1 placeholder
    ...
    python3 $ROOT/tools/p8xfs.py put mc0.img mc_dir.bin --name /bin/dir.bin --load 0x5900 --exec 0x5900 >/dev/null
    python3 $ROOT/tools/p8xfs.py put mc0.img mc_cat.bin --name /bin/cat.bin --load 0x5900 --exec 0x5900 >/dev/null
}
