...
python3 $ROOT/assembler/p8xasm.py cmp.t.asm -o cmp.t.bin --base 0x6300 >/dev/null
rm -f cmp.img
...
python3 $ROOT/tools/p8xfs.py put    cmp.img cmp.t.bin --name /bin/cmp.bin --load 0x6300 --exec 0x6300 >/dev/null
printf 'hello world\nsecond line\n' > A.TXT
printf 'hello world\nsecond line\n' > B.TXT
printf 'hello worle\nsecond line\n' > C.TXT
printf 'hello world\n'              > D.TXT
...
python3 $ROOT/assembler/p8xasm.py cmp.a.asm -o cmp.a.bin --base 0x6300 >/dev/null
rm -f cmpa.img
...
python3 $ROOT/tools/p8xfs.py put    cmpa.img cmp.a.bin --name /bin/cmp.bin --load 0x6300 --exec 0x6300 >/dev/null
