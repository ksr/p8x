...
cat > prog.asm <<'EOF'
        ...
        RTS
EOF
python3 $ROOT/assembler/p8xasm.py prog.asm -o prog.bin --base 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py os_dir.asm -o os_dir.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py mkdir os.img /bin >/dev/null
python3 $ROOT/tools/p8xfs.py put os.img os_dir.bin --name /bin/dir.bin --load 0x5900 --exec 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py os_del.asm -o os_del.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put os.img os_del.bin --name /bin/del.bin --load 0x5900 --exec 0x5900 >/dev/null
printf 'hi' > os_h.tmp
