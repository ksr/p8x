...
run() {   # $1 = asm file -> emulator output (letters+digits)
    python3 $ROOT/assembler/p8xasm.py "$1" -o lf.bin --base 0x6300 >/dev/null
    rm -f lf.img
    python3 $ROOT/tools/p8xfs.py create lf.img >/dev/null
    python3 $ROOT/tools/p8xfs.py boot   lf.img osc.bin >/dev/null
    python3 $ROOT/tools/p8xfs.py put    lf.img lf.bin --name LF.bin --load 0x6300 --exec 0x6300 >/dev/null
