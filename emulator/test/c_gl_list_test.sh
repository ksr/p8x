...
for n in la lb lc ld; do
    ...
    python3 $ROOT/assembler/p8xasm.py gl_$n.asm -o gl_$n.bin --base 0x5900 >/dev/null
done
rm -f gll.img
...
for n in la lb lc ld; do
    python3 $ROOT/tools/p8xfs.py put gll.img gl_$n.bin --name /bin/$n.bin --load 0x5900 --exec 0x5900 >/dev/null
done
...
# the pure scene, no always-on console): GCONEN -> 00 (console off) -> . ; G 014E (GCLS).
    printf "E ${GCONEN}\r00.G 014E\rB\rrun /bin/%s.bin %s\r" "$1" "$3" > gl_l.in
