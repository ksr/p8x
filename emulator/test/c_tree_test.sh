...
build_disk() {   # $1 = py|host
    ...
    python3 $ROOT/assembler/p8xasm.py tree.asm -o tree.bin --base 0x5900 >/dev/null
    rm -f tr.img
    ...
    python3 $ROOT/tools/p8xfs.py put tr.img tree.bin --name /bin/tree.bin --load 0x5900 --exec 0x5900 >/dev/null
    printf 'z' > tr_z.dat
