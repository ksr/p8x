...
build_disk() {   # $1 = cat.bin
    ...
    python3 $ROOT/tools/p8xfs.py put    cat.img "$1" --name /bin/cat.bin --load 0x6300 --exec 0x6300 >/dev/null
    printf 'ROOTOK\n' > rr.txt
...
if command -v cc >/dev/null 2>&1; then
    ...
    python3 $ROOT/assembler/p8xasm.py ch.asm -o ch.bin --base 0x6300 >/dev/null
    build_disk ch.bin
    check "p8cc.c"
fi

python3 $ROOT/compiler/p8cc.py cat.pp.c -o cp.asm >/dev/null
python3 $ROOT/assembler/p8xasm.py cp.asm -o cp.bin --base 0x6300 >/dev/null
build_disk cp.bin
check "p8cc.py"
