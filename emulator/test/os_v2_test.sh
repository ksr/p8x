...
cat > v2prog.asm <<'EOF'
        ...
        RTS
EOF
python3 $ROOT/assembler/p8xasm.py v2prog.asm -o v2prog.bin --base 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py v2dep.asm -o v2dep.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put    v2.img v2dep.bin --name /bin/dep.bin --load 0x5900 --exec 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py v2cat.asm -o v2cat.bin --base 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put    v2.img v2cat.bin --name /bin/cat.bin --load 0x5900 --exec 0x5900 >/dev/null
...
for c in dir pwd tree del; do
    ...
    python3 $ROOT/assembler/p8xasm.py v2$c.asm -o v2$c.bin --base 0x5900 >/dev/null
    up=$(echo $c | tr a-z A-Z)
    python3 $ROOT/tools/p8xfs.py put v2.img v2$c.bin --name /bin/$c.bin --load 0x5900 --exec 0x5900 >/dev/null
done
