...
build_disk() {   # $1 = py|host
    for c in find diff; do
        ...
        python3 $ROOT/assembler/p8xasm.py $c.asm -o $c.bin --base 0x6300 >/dev/null
    done
    rm -f fd.img
    ...
    python3 $ROOT/tools/p8xfs.py put fd.img find.bin --name /bin/find.bin --load 0x6300 --exec 0x6300 >/dev/null
    python3 $ROOT/tools/p8xfs.py put fd.img diff.bin --name /bin/diff.bin --load 0x6300 --exec 0x6300 >/dev/null
