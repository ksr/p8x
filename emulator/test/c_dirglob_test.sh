...
build_disk() {   # $1 = py|host
    ...
    python3 $ROOT/assembler/p8xasm.py dg.asm -o dg.bin --base 0x6300 >/dev/null
    rm -f dg.img
    ...
    python3 $ROOT/tools/p8xfs.py put    dg.img dg.bin --name /bin/dir.bin --load 0x6300 --exec 0x6300 >/dev/null
    printf x > dgf
