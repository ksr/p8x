...
python3 $ROOT/assembler/p8xasm.py cbfull.asm -o cbasm.bin --base 0x6100 >/dev/null
python3 $ROOT/assembler/p8xasm.py $ROOT/apps/p8xcc.asm -o cbcc.bin --base 0x6100 >/dev/null

# --- asm path: the vi twin, spliced host-side (mkasm) so the disk is self-contained
sh $ROOT/os/commands-asm/mkasm.sh vi > vitwin.asm
python3 $ROOT/assembler/p8xasm.py vitwin.asm -o vigold.bin --base 0x6100 >/dev/null
...
python3 $ROOT/tools/p8xfs.py put    cb.img cbasm.bin --name ASM.bin --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put    cb.img cbcc.bin  --name CC.bin  --load 0x6100 --exec 0x6100 >/dev/null
