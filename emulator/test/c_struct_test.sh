...
python3 $ROOT/assembler/p8xasm.py cstruct.asm -o cstruct.bin --base 0x6100 >/dev/null

rm -f cs.img
python3 $ROOT/tools/p8xfs.py create cs.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   cs.img osc.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    cs.img cstruct.bin --name CS.bin --load 0x6100 --exec 0x6100 >/dev/null
