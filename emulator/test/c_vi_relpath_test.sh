...
build_disk() {   # $1 = vi binary -> vi.img with /SUB/READ.TXT + /VI.bin
    ...
    python3 $ROOT/tools/p8xfs.py put vi.img "$1" --name /VI.bin --load 0x6100 --exec 0x6100 >/dev/null
}
...
python3 $ROOT/assembler/p8xasm.py vi.asm -o vi.bin --base 0x6100 >/dev/null
build_disk vi.bin
check "vi.c (p8cc)"

# asm twin
sh $ROOT/os/commands-asm/mkasm.sh vi > via.asm
python3 $ROOT/assembler/p8xasm.py via.asm -o via.bin --base 0x6100 >/dev/null
build_disk via.bin
check "vi.asm (hand)"
