...
check() {   # $1 = label, $2 = prod.asm, $3 = cat.asm
    python3 $ROOT/assembler/p8xasm.py "$2" -o prod.bin --base 0x5900 >/dev/null
    python3 $ROOT/assembler/p8xasm.py "$3" -o cat.bin  --base 0x5900 >/dev/null
    rm -f pp.img
    python3 $ROOT/tools/p8xfs.py create pp.img >/dev/null
    python3 $ROOT/tools/p8xfs.py boot   pp.img osc.bin >/dev/null
    python3 $ROOT/tools/p8xfs.py put    pp.img prod.bin --name PROD.bin --load 0x5900 --exec 0x5900 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    pp.img cat.bin  --name CAT.bin  --load 0x5900 --exec 0x5900 >/dev/null
