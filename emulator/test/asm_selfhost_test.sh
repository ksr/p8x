...
python3 $ROOT/assembler/p8xasm.py shfull.asm -o shgold.bin --base 0x6100 >/dev/null

rm -f sh.img
python3 $ROOT/tools/p8xfs.py create sh.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   sh.img ossh.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    sh.img shgold.bin --name ASM.bin --load 0x6100 --exec 0x6100 >/dev/null
