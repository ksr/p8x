...
for p in wdesk paint; do
    ...
    python3 $ROOT/assembler/p8xasm.py wd_$p.asm -o wd_$p.bin --base 0x6300 >/dev/null
done
...
python3 $ROOT/tools/p8xfs.py put    wd.img wd_wdesk.bin --name /bin/wdesk.bin --load 0x6300 --exec 0x6300 >/dev/null
python3 $ROOT/tools/p8xfs.py put    wd.img wd_paint.bin --name /bin/paint.bin --load 0x6300 --exec 0x6300 >/dev/null
