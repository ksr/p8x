...
python3 $ROOT/assembler/p8xasm.py ctest.asm -o ctest.bin --base 0x5900 >/dev/null

rm -f c.img
python3 $ROOT/tools/p8xfs.py create c.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   c.img osc.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    c.img ctest.bin --name CT.bin --load 0x5900 --exec 0x5900 >/dev/null
