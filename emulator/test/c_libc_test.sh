...
python3 $ROOT/assembler/p8xasm.py clibc.asm -o clibc.bin --base 0x6100 >/dev/null

rm -f cl.img
python3 $ROOT/tools/p8xfs.py create cl.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   cl.img osc.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    cl.img clibc.bin --name CL.bin --load 0x6100 --exec 0x6100 >/dev/null
