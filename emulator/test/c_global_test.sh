...
python3 $ROOT/assembler/p8xasm.py cglob.asm -o cglob.bin --base 0x6300 >/dev/null

rm -f cg.img
python3 $ROOT/tools/p8xfs.py create cg.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   cg.img osc.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    cg.img cglob.bin --name CG.bin --load 0x6300 --exec 0x6300 >/dev/null
