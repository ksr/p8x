...
python3 $ROOT/assembler/p8xasm.py $ROOT/apps/p8xcc.asm -o cc.bin --base 0x6100 >/dev/null
...
build_c() { # build_c <src.c> <out.bin>
    ...
    python3 $ROOT/assembler/p8xasm.py tmp.asm -o "$2" --base 0x6100 >/dev/null 2>&1
}
build_c $ROOT/os/commands/cat.c cat.bin
build_c $ROOT/os/commands/cp.c  cp.bin
build_c $ROOT/os/commands/del.c del.bin   # `make clean` recipe runs `del` (a /bin program now)
...
python3 $ROOT/tools/p8xfs.py put mk.img cc.bin  --name /bin/cc.bin  --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put mk.img asm.bin --name /bin/asm.bin --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put mk.img cat.bin --name /bin/cat.bin --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put mk.img cp.bin  --name /bin/cp.bin  --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put mk.img del.bin --name /bin/del.bin --load 0x6100 --exec 0x6100 >/dev/null
printf 'hi\n' > HELLO.TXT
