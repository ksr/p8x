...
python3 $ROOT/assembler/p8xasm.py examine.c.asm -o examine.c.bin --base 0x5900 >/dev/null
...
python3 $ROOT/assembler/p8xasm.py dump.x.asm -o dump.x.bin --base 0x5900 >/dev/null
# asm twin: mkasm splices ;#use, then assemble
sh $ROOT/os/commands-asm/mkasm.sh examine > examine.a.asm
python3 $ROOT/assembler/p8xasm.py examine.a.asm -o examine.a.bin --base 0x5900 >/dev/null
...
mkdisk() { # mkdisk <img> <examine.bin>
    ...
    python3 $ROOT/tools/p8xfs.py put    "$1" "$2"        --name /bin/examine.bin --load 0x5900 --exec 0x5900 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    "$1" dump.x.bin  --name /bin/dump.bin    --load 0x5900 --exec 0x5900 >/dev/null
}
mkdisk exc.img examine.c.bin
mkdisk exa.img examine.a.bin

# $C000 is clear of the program at $5900. examine auto-advances after each pair
