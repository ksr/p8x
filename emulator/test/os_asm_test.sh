...
# hashed symbol table right after itself. 0x8000-0x5900 = 7936 bytes.
[ "$(wc -c < asm.bin)" -le 9984 ] || { echo "OS-ASM TEST: FAIL — asm.bin is $(wc -c < asm.bin) bytes, overlaps SYMTAB at \$8000"; exit 1; }
...
L=["VAL = $1234", "CH  = 'Q'", "        .org $5900", "begin:"]
...
PYEOF
python3 $ROOT/assembler/p8xasm.py cover.asm -o covgold.bin --base 0x5900 >/dev/null
...
esc:    .asciiz "T\tQ\"B\\N\n"
EOF
python3 $ROOT/assembler/p8xasm.py prog.asm -o golden.bin --base 0x5900 >/dev/null

rm -f as.img
python3 $ROOT/tools/p8xfs.py create as.img >/dev/null
python3 $ROOT/tools/p8xfs.py boot   as.img osa.bin >/dev/null
python3 $ROOT/tools/p8xfs.py put    as.img asm.bin --name ASM.bin --load 0x5900 --exec 0x5900 >/dev/null
