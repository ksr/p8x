...
build_disk() {   # $1 = py|host
    ...
    python3 $ROOT/tools/p8xfs.py put    fo.img cp.bin --name /bin/cp.bin --load 0x6100 --exec 0x6100 >/dev/null
    python3 $ROOT/tools/p8xfs.py put    fo.img mv.bin --name /bin/mv.bin --load 0x6100 --exec 0x6100 >/dev/null
