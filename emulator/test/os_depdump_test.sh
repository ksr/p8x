...
for c in dep dump; do
    ...
    python3 $ROOT/assembler/p8xasm.py $c.dd.asm -o $c.dd.bin --base 0x6A00 >/dev/null
done
...
python3 $ROOT/tools/p8xfs.py put    dd.img dep.dd.bin  --name /bin/dep.bin  --load 0x6A00 --exec 0x6A00 >/dev/null
python3 $ROOT/tools/p8xfs.py put    dd.img dump.dd.bin --name /bin/dump.bin --load 0x6A00 --exec 0x6A00 >/dev/null

# bare-name dep/dump (no `run`), proving the built-ins are gone. $9000 is clear
# of the program at $6A00. '.' exits dump's pager.
