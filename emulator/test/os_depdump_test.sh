...
for c in dep dump; do
    ...
    python3 $ROOT/assembler/p8xasm.py $c.dd.asm -o $c.dd.bin --base 0x5900 >/dev/null
done
...
python3 $ROOT/tools/p8xfs.py put    dd.img dep.dd.bin  --name /bin/dep.bin  --load 0x5900 --exec 0x5900 >/dev/null
python3 $ROOT/tools/p8xfs.py put    dd.img dump.dd.bin --name /bin/dump.bin --load 0x5900 --exec 0x5900 >/dev/null

# bare-name dep/dump (no `run`), proving the built-ins are gone. $9000 is clear
# of the program at $5900. '.' exits dump's pager.
