#!/bin/sh
# Text-overlay cross-implementation pixel proof: the SAME TX* + background GL
# byte stream is composited by the EMULATOR (gl_tx_pixel into its -g PPM) and by
# the RTL (tb_gl_ovx.v: real p8x_geom + gfx + arbiter + p8x_sdram + sdram_video
# WITH its gtxt, captured at the SCANOUT PINS -- the overlay composites there, so
# the SDRAM dump the other tb_gl_* benches use would not see it). The two
# 480x272 frames must be BYTE-IDENTICAL: enable, window, colour, glyphs, cursor
# advance and TXSCR all agreeing on every pixel. Needs iverilog on PATH.
set -e
set -o pipefail
cd "$(dirname "$0")"
ROOT=../..
...
echo "C-GL-OVL-RTL TEST: PASS (TX* overlay: RTL scanout == emulator, byte-identical)"
