...
build() {   # $1 = command name -> $1.bin
    ...
    python3 $ROOT/assembler/p8xasm.py "$1.d.asm" -o "$1.d.bin" --base 0x6100 >/dev/null
}
build disasm
build dep
...
python3 $ROOT/tools/p8xfs.py put disasm.img disasm.d.bin --name /bin/disasm.bin --load 0x6100 --exec 0x6100 >/dev/null
python3 $ROOT/tools/p8xfs.py put disasm.img dep.d.bin    --name /bin/dep.bin    --load 0x6100 --exec 0x6100 >/dev/null
