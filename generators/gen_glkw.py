...
# ---- the C-written BASIC (basic/basic.c) gets the same two tables as a C
# fragment, concatenated ahead of the source at build time (no #include).
with open(os.path.join(HERE, "..", "basic", "glkwtab.c"), "w") as f:
    f.write("/* glkwtab.c - GENERATED by generators/gen_glkw.py. Do not edit.\n")
    f.write(" * The GL verbs as BASIC keywords: glkw[] is [chars][token]... 0, listed\n")
    f.write(" * LONGEST-FIRST (tokens $%02X up in BASIC_VERBS order, the .BAS ABI);\n" % GLV0)
    f.write(" * glvtab[] is two bytes per verb in token order: GL opcode, meta. */\n")
    f.write("//#define GLV0 %d\n" % GLV0)
    f.write("//#define GLVN %d\n" % len(bverbs))
    f.write("char glkw[] = {\n")
    for i in order:
        f.write("    " + ",".join("'%s'" % ch for ch in bverbs[i][0]) + ", 0x%02X,\n" % (GLV0 + i))
    f.write("    0 };\n")
    f.write("char glvtab[] = {\n")
    for i, (lng, op, b, a) in enumerate(bverbs):
        if a == 15:
            ...
            meta = (b << 4) | (a - b)
        f.write("    0x%02X, 0x%02X,   /* $%02X %s */\n" % (op, meta, GLV0 + i, lng))
    f.write("    0 };\n")
print("wrote basic/glkwtab.c")
