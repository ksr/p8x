...
DESC = {
    ...
    ("PHW", "(P1+d)"): ("CZN", "Push the word at P1+d (hi then lo). A!; flags from the address add."),
    ("PHW", "(P2+d)"): ("CZN", "Push the word at P2+d. A!"),
    ("PHW", "(P3+d)"): ("CZN", "Push the word at P3+d, d measured before the push (a local/arg onto the stack). A!"),
    ...
    # relative branches: 2 bytes; signed d8 from the next instruction. TAKEN path
    # clobbers A and the flags (8 steps); not taken touches nothing (2 steps).
    ("JMP", "r"): ("CZNV", "P0:=P0+rel8 (2-byte jump). A! flags!; assembler .relax / JMP.R. Always-taken jumps are cheaper absolute (JMP.A, 3 steps)."),
    ("BZ", "r"): ("CZNV", "Branch rel8 if Z=1. Taken: A! flags!. (JZ.R alias.)"),
    ("BNZ", "r"): ("CZNV", "Branch rel8 if Z=0. Taken: A! flags!. (JNZ.R alias.)"),
    ("BCP", "r"): ("CZNV", "Branch rel8 if C=1. Taken: A! flags!. (JC.R alias.)"),
    ("JNC", "r"): ("CZNV", "Branch rel8 if C=0. Taken: A! flags!."),
    ("BLT", "r"): ("CZNV", "Branch rel8 if signed A<B (N^V). Taken: A! flags!."),
    ("BGE", "r"): ("CZNV", "Branch rel8 if signed A>=B. Taken: A! flags!."),
    ("BLE", "r"): ("CZNV", "Branch rel8 if signed A<=B. Taken: A! flags!."),
    ("BGT", "r"): ("CZNV", "Branch rel8 if signed A>B. Taken: A! flags!."),
