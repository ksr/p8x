...
Usage: gen_p8xopc.py [out.asm] [--c]   (defaults to stdout; --c, or a file
       output, also writes apps/opctab.c for the C-written assembler)
"""
import sys, os
...
def main():
    ...
    out = open(files[0], "w") if files else sys.stdout
    w = out.write
    w("; ============================================================\n")
    ...
    if out is not sys.stdout:
        out.close()
    # the same table as a C fragment for the C-written assembler (apps/asm.c):
    # concatenated ahead of the source at build time (there is no #include)
    if "--c" in sys.argv or out is not sys.stdout:
        cp = os.path.join(HERE, "..", "apps", "opctab.c")
        with open(cp, "w") as f:
            f.write("/* opctab.c - GENERATED by generators/gen_p8xopc.py. Do not edit.\n")
            f.write(" * [shape][opcode][mnemonic chars]0 ... 0xFF, sorted by mnemonic. */\n")
            f.write("char opctab[] = {\n")
            for (mn, sh), op in sorted(OPC.items()):
                if sh in HOST_ONLY:
                    continue
                f.write("    %d, 0x%02X, %s, 0,\n" % (SHAPE[sh], op, ",".join("'%s'" % c for c in mn)))
            f.write("    0xFF };\n")
