// tb_gl_apx.v -- stage-10a GL stream through the REAL pixel stack, and the
...
module tb;
  ...
  endtask
  task gls(input [1023:0] str);      // send a NUL-padded ASCII string
    integer si;
    begin
      for (si = 127; si >= 0; si = si - 1)
        if (str[si*8 +: 8] != 8'h00) glb(str[si*8 +: 8]);
    end
  endtask
  ...
  task gl_wait_idle;
    ...
    begin
      ...
      gl_sel = 0;
      if (n >= 4000000) begin $display("TB-GL-APX: FAIL (never idle)"); $finish(1); end
  ...
  initial begin
    ...
    // ---- the SAME scene as ASCII TEXT (stage 10d): the fabric
    // translator must produce the hex frame byte for byte -------------
    glb(8'h43); glb(8'h41); glb(8'h20);             // hex "CA "
    gls("WINDOW -120 120 -120 120\015");
    gls("VWPORT 104 375 0 271\015");
    gls("FLOOD 0 0 0\015");
    gls("C 31 63 31 M3 -90 -90 300 D3 90,-90,300\015");
    gls("C 31 0 0 M3 90 -90 300 D3 0 90 300\015");
    gls("c 0 63 0 m3 -200 0 300 d3 200 50 300\015");
    gls("C 0 0 31 M3 0 0 -50 D3 0 0 300\015");
    gls("PF 1 C 31 63 0\015");
    gls("P3 3 -80 -80 300 80 -80 300 0 40 420\015");
    gls("C 0 63 31\015");
    gls("P3 3 100 -140 260 140 60 260 -40 10 200\015");
    gls("CX ");
    gl_wait_idle;

    if (CHIP.protocol_errors != 0) begin
      $display("TB-GL-APX: FAIL (%0d protocol errors)", CHIP.protocol_errors);
      $finish(1);
    end

    // ---- dump page 0 as a P6 PPM, the emulator's exact format -------------
    f = $fopen("tb_gl_apx.ppm", "wb");
    $fwrite(f, "P6\n480 272\n255\n");
    for (y = 0; y < 272; y = y + 1)
    ...
    $fclose(f);
    $display("TB-GL-APX: DONE (tb_gl_apx.ppm written; compare against the emulator)");
    $finish(0);
  end

  initial begin #400_000_000; $display("TB-GL-APX: TIMEOUT"); $finish(1); end
endmodule
