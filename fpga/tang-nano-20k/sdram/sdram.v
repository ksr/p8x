...
// questions -- the same reason the C emulator is the CPU's golden model. A P8X
// controller written for the framebuffer can be diffed against this one.
//
// Interface: 32-bit data bus to the chip, byte-wide to the caller. Commands are
// one-cycle pulses issued while !busy; a read answers with data_ready and dout.
// `refresh` is the CALLER's responsibility -- see the refresh timer in
// sdram_test.v.
