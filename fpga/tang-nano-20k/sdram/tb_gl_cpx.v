// tb_gl_cpx.v -- stage-10a GL stream through the REAL pixel stack, and the
...
//
//   iverilog -g2012 -o tbglp tb_gl_cpx.v ../../rtl/p8x_geom.v \
...
module tb;
  ...
  task gl_wait_idle;
    ...
    begin
      ...
      gl_sel = 0;
      if (n >= 4000000) begin $display("TB-GL-CPX: FAIL (never idle)"); $finish(1); end
...
initial begin
    ...
    gpoke(4'h8, 8'd80);  gpoke(4'hF, 8'd40);
    gpoke(4'h5, 8'h0A); gwait;                          // ellipse 80x40
    gpoke(4'h4, 8'h1F); gpoke(4'hD, 8'h00);             // blue
    gpoke(4'h0, 8'd100); gpoke(4'h9, 8'd0);
    gpoke(4'h1, 8'd200); gpoke(4'hA, 8'd0);
    gpoke(4'h8, 8'd25);  gpoke(4'hF, 8'd60);
    gpoke(4'h5, 8'h0B); gwait;                          // ellipse fill 25x60
    gpoke(4'h4, 8'hFF); gpoke(4'hD, 8'hFF);             // white
    ...
    if (CHIP.protocol_errors != 0) begin
      $display("TB-GL-CPX: FAIL (%0d protocol errors)", CHIP.protocol_errors);
      $finish(1);
    end

    // ---- dump page 0 as a P6 PPM, the emulator's exact format -------------
    f = $fopen("tb_gl_cpx.ppm", "wb");
    $fwrite(f, "P6\n480 272\n255\n");
    for (y = 0; y < 272; y = y + 1)
    ...
    $fclose(f);
    $display("TB-GL-CPX: DONE (tb_gl_cpx.ppm written; compare against the emulator)");
    $finish(0);
  end

  initial begin #400_000_000; $display("TB-GL-CPX: TIMEOUT"); $finish(1); end
endmodule
