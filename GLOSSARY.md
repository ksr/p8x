...
| **Screen geometry** | 480×272 at 8 bpp — 256 pens — and nothing else. The display was briefly two modes (240×136 at 2 bpp, and this one) selected by `GCMD $0C` SETMODE; both SETMODE and BASIC's `SCREEN` were removed once the SDRAM framebuffer made the larger mode the only one worth having. |
| **3-3-2 ramp** | The power-on palette: a pen's own bits are its colour — three of red, three of green, two of blue — so pen 224 is red and pen 28 green before `PALETTE` is called. `PALETTE` then maps any of the 256 pens to any of 4096 colours. |
