...
| **geometry engine** | The wireframe-3D walker at `$FF40` (stage 8b): edge list in SDRAM at `$100000`, S7.8 matrix + translation in an indexed parameter file, one RENDER command to transform/near-clip/project/window-clip/map and draw by mastering the display's own registers. Probe: `GEID` reads `'E'`. |
...
| **typed records** | The geometry engine's list format (stage 9): type byte, flags, an RGB565 colour, then vertices — LINE (16 bytes) and TRI (22, outline or filled). Self-sizing; the walker advances by type. |
| **scene** | The engine's persisted state between commands: the record list in SDRAM plus the parameter file (readable since 9c). `tri` builds it, `tri …k` appends, `rotate`/`camera` re-render it. |
