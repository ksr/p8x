...
| **GL / graphics language** | The stage-10 command port at `$FF50` (STAGE10-DESIGN.md, modelled on the Matrox PG-640A): opcode + int16 parameters into a FIFO; primitives, card-side modeling/viewing matrices, projection, hither/yon clipping, pages, command lists. `man gl` on-target is the reference card. |
