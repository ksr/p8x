...
| Term | Meaning |
|------|---------|
| **P8X** | The project: a hand-built 8-bit microcoded TTL CPU (~130 74HCT chips, 8-slot DIN 41612 backplane) — and, since 2026-08, the same microarchitecture on an FPGA. |
...
| **pen** | The current drawing colour, a whole RGB565 value since direct colour (stages 5-6). ONE pen since the single-interface migration: the GL walker's (`COLOR` verb) — the walker loads it into its private register file while drawing. BASIC's `COLOR` statement and the C library's `gcolor()` both just emit the verb. |
...
| **drawing engine** | The state machine *in the card* that renders a primitive from the walker's register file. Why a filled box is a few GL stream bytes and not 32640 read-modify-writes. |
