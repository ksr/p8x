...
| Term | Meaning |
|------|---------|
| **framebuffer** | The pixel memory the display scans out of: 480x272 at 16 bpp RGB565, two pages in the Tang Nano's in-package SDRAM behind the streaming controller (FLIP swaps them). The 240x136/2 bpp BSRAM framebuffer was the stage-4 original. |
| **pen** | The current drawing colour, a whole RGB565 value since direct colour (stages 5-6). Three pens exist: the device's write-only `GCOL`/`GCOLH` pair, the GL walker's pen (`COLOR` verb — and the walker rewrites the device pair internally while drawing), and BASIC's `GPEN` shadow, the pen of record its `COLOR` statement sets. |
| **palette** | RETIRED (stages 5-6, with the four-pen modes): the table that mapped each pen to a 12-bit RGB value. Direct colour has no palette — `SETPAL` and BASIC's `PALETTE` went with it. |
| **scanout** | The read side: `sdram_video.v` fetching each panel line from the SDRAM framebuffer through the controller's stream port into a ping-pong line buffer, and emitting RGB + DE. (`video_rgb.v` and the shared-port `sc_en` era belong to the retired BSRAM framebuffer.) |
...
| **geometry engine** | Historically the record walker at `$FF40` (stage 8b, RETIRED at stage 10b); today the term means the GL engine behind the graphics language at `$FF50` — same transform/clip/draw pipeline, driven by a command stream instead of registers. Probe: `GLID` reads `'G'`. |
| **GL / graphics language** | The stage-10 command port at `$FF50` (STAGE10-DESIGN.md, modelled on the Matrox PG-640A): opcode + int16 parameters into a FIFO; primitives, card-side modeling/viewing matrices, projection, hither/yon clipping, pages, command lists. Two encodings, switchable with `CA`/`CX`: hex opcodes, or ASCII keywords parsed IN FABRIC (stage 10d). Reachable three ways: the `gl` console command, BASIC's `GL s$` string statement, and — since 10d — the verbs as NATIVE BASIC statements (`MDROTY A`, `POLY3 3,...`; tokens generated by gen_glkw.py, the same table the RTL and emulator use). `man gl` on-target is the reference card. |
| **page flip** | Double buffering: two 512K-aligned framebuffer pages (address bit 19). The engine owns the state; a flip latches at the scanout frame boundary with the rule display ← draw, draw ← other. Power-on page contents are UNDEFINED — clear both before flipping — and `PGSYNC` (GECMD 4 / `g3sync()`) is the only way back to single-buffer. |
| **typed records** | The retired stage-9 list format (type byte, flags, RGB565 colour, vertices). Superseded at stage 10 by the GL command stream — a scene is now a command list, not a record list. |
| **scene** | GL command list 0 (stage 10c): the byte stream `tri` records (`tri …k` appends via CLAPP) and `rotate`/`camera` replay with new matrices. Stored on the card in SDRAM; the card's matrix/window state persists alongside it. |
| **command list** | A GL byte stream stored on the card (CLBEG…CLEND, 256 slots of 4KB at `$100000`), replayed by CLRUN/CLOOP. Matrix deltas inside a looped list accumulate per pass — the fly-through/self-spinning-demo mechanism. |
...
| **GTEXT** | BASIC's graphics-text statement. The one drawing operation NOT done by the device — there is no text command, so BASIC walks a 5×7 glyph and issues one `PIXELW`/`BOXFILL` per lit pixel, in the pen it asserts from `GPEN`. Window coordinates, baseline anchor, like every BASIC graphics statement. |
...
| **GPEN** | BASIC's pen of record (`COLOR` writes it). The device `GCOL` is write-only and the GL walker clobbers it while drawing, so `GTEXT` asserts this shadow into `GCOL`/`GCOLH` at entry; nothing else writes the device pen directly. |
