...
| `$FF20–$FF2F` | graphics display (**no card built yet** — planned as a Tang Nano 20K + 4.3" panel on the bus; the FPGA P8X implements the identical device internally). A 240×136 four-colour framebuffer with a drawing engine. `GCMD` executes: 1 `PLOT`, 2 `LINE`, 3 `BOX`, 4 `BOXFILL`, 5 `CLS`, 6 `SETPAL`, 7 `CIRCLE`, 8 `CIRCLEFILL`, 9 `POINT`, `$0A` `ELLIPSE`, `$0B` `ELLIPSEFILL`, `$F1` `RESET`, `$F2` `IDENT` (and `$F0` `SELFTEST`, which the emulator implements but the RTL rejects — see [BACKLOG.md](../BACKLOG.md)). `GID0`/`GID1` read `$50`/`$47` ("PG") for presence, since an absent card floats to `$FF`. Modelled by `p8xemu` — see [emulator/README.md](../emulator/README.md#the-graphics-display) |
