...
description: CWDPATH is 48 bytes at $5800 (was $6400 before the 2026-09-13 TPABASE drops) and is NOT just prompt text — it feeds SYS_GETCWD and DERIVEDRV; overflow lands on INMODE/INARM/CWDLH
metadata: 
  node_type: memory
  type: reference
...
`CWDPATH` ($5800, **48 bytes**) is not "the prompt string", despite what the code
