...
- [Full 3D stack + GL SHIPPED](project_p8x_gfx_clib.md) — stage 10 GL ($FF50) COMPLETE ON SILICON: 10a-k all board-proven 2026-08-30 (10e read-back RESURRECTED a7f3206 — FLAGRD/MATXRD/CLRD/CLMOD live, clsave is its payoff); fit endgame: practical placement cliff ~19,150 LUT4, AREAPT (−569) and ARC/SECTOR (−2,045) removed + BACKLOG'd, card places 18,065/87% seed 1; card personality IN FLASH (no CPU on chip), emulator=CPU via p8xemu -B/runcard.sh; SINGLE-INTERFACE: PIXRD on silicon (PIXELR()=GL, card 19,103/92% at the cliff), GTEXT retired for PGC TEXT + boot-streamed /FONT.GL (idiom MOVE3 x,y,0 : TEXT s$; cold-start PROJCT 0; TSIZE/TANGLE COMPOUND like matrix verbs -- MDIDEN resets, 256 is a no-op; anchor scales too); $FF20 BASIC-free (BLIT shipped 6768a9b: IMAGE=1 BLIT/row, CLMOD removed -569... -342 funded it, card 18,916/91%, ON SILICON: mandrill IMAGE 393.5s->37.7s = 10.4x); next: C lib, monitor splash, door closes
