...
- [Resident WM roadmap](project_p8x_resident_wm.md) — kernel FOLDED INTO the OS as syscalls $2027-$2039 (no load step); `wdesk` = thin launcher on it, paint `-w` resumes it (desktop survives app launch, c_wdesk_test); history ring 32→8 lines at $F800..$F9FF (the "free" reserve at $5800 held it; then $FA00 FSDIRBUF page collided). Next: migrate menu/close/focus/FILES/TERM into the kernel, then per-window tty/pty. LESSONS: grep gen_memmap.py anchors AND lib_*.c 0x... #defines before calling any region free.
- [Full 3D stack + GL SHIPPED](project_p8x_gfx_clib.md) — stage 10 GL ($FF50) COMPLETE ON SILICON: 10a-k all board-proven 2026-08-30 (10e read-back RESURRECTED a7f3206 — FLAGRD/MATXRD/CLRD/CLMOD live, clsave is its payoff); fit endgame: practical placement cliff ~19,150 LUT4, AREAPT (−569) and ARC/SECTOR (−2,045) removed + BACKLOG'd, card places 18,065/87% seed 1; card personality IN FLASH (no CPU on chip), emulator=CPU via p8xemu -B/runcard.sh; SINGLE-INTERFACE: PIXRD on silicon (PIXELR()=GL, card 19,103/92% at the cliff), GTEXT REBORN a482764 as 2D sugar (old signature x,y,size,s$, token $AF, PURE GL: PROJCT 0/MDIDEN/TSIZE/MDTRAN/MOVE3/TEXT -- MDTRAN-after-TSIZE = unscaled anchor; resets matrix+camera per call BY DESIGN); raw idiom MOVE3 x,y,0 : TEXT s$ for 3D (TSIZE/TANGLE COMPOUND, MDIDEN resets); $FF20 BASIC-free (BLIT shipped 6768a9b: IMAGE=1 BLIT/row, CLMOD removed -569... -342 funded it, card 18,916/91%, ON SILICON: mandrill IMAGE 393.5s->37.7s = 10.4x); next: C lib, monitor splash, door closes
