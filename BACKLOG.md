...
- [ ] **Faster image transfer (2026-08-29, user; REGRESSION noted
      ...
      39.09M cycles at 66fe59e (pre-migration) -- byte-for-byte the
      same CPU cost (~596 c/px), so the CODE did not regress. The
      regression is ARCHITECTURAL: the card arc moved the CPU off the
      chip, so what used to be on-chip register writes now cross the
